...
function clickHandler(event) {
    ...
    let deviceCategory = event.target.dataset.type;

    showCategory(deviceCategory);
}
...
function showCategory(category) {
    let devices = products[category];
    for (let i = 0; i < devices.length; i++) {
        productsEl.insertAdjacentHTML('beforeend', getProductMarkup(devices[i]));
    }
}
...
function getProductMarkup(product) {
    let productMarkup = `
        <div class="product">
            ...
            <img src="https://picsum.photos/seed/${product.id}/200" alt="">
